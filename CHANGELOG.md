...
- Patch `0052`: an eligible object literal's field stores are fused at compile
  time into one `OP_object_fill`, deleting N-1 `OP_define_field` dispatches and
  keeping the values on the operand stack until the fill. A 3-property literal is
