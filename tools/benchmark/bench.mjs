...
function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (a === "--rows") out.rows.push(argv[++i]);
      ...
      const nx = argv[i + 1];
      out.save = nx && !nx.startsWith("--") ? (i++, nx) : true;
...
function allRows(suites) {
  ...
  for (const s of suites) {
    if (s.kind === "workload") {
      ...
      for (const f of readFileSync(
        join(OCTANE, "corpus", "MANIFEST.json"),
        "utf8",
      ).matchAll(/"row":\s*"([^"]+)"/g))
      ...
      for (const sh of knownCalls())
        out.push({ id: `calls/${sh}`, suite: "calls", name: sh });
    } else if (s.kind === "startup") {
      for (const p of ["load-only", "register-only", "entry", "force-all"])
        out.push({ id: `startup/${p}`, suite: "startup", name: p });
    }
  }
  return out;
}
...
function knownCalls() {
  ...
  try {
    _knownCalls = execFileSync(binPath("native-call-bench"), ["--list"], {
      encoding: "utf8",
    })
      .trim()
      .split("\n");
...
function binPath(name) {
  const candidates = [
    join(ROOT, "build-release", name),
    join(ROOT, "build", name),
  ];
  for (const p of candidates) if (existsSync(p)) return p;
  process.stderr.write(
    `[bench] ${name} not found; building Release binaries...\n`,
  );
  execFileSync(
    "cmake",
    ["-B", join(ROOT, "build-release"), "-DCMAKE_BUILD_TYPE=Release"],
    { stdio: "inherit" },
  );
  execFileSync(
    "cmake",
    ["--build", join(ROOT, "build-release"), "-j8", "--target", name],
    { stdio: "inherit" },
  );
...
function launch(engine, argv, opts = {}) {
  if (engine.device && engine.kind === "hermes")
    return device.deviceHermesRun(engine.device.hermes, argv);
  if (engine.device) return device.deviceRun(engine.device.qjs, argv);
  return spawnSync(engine.bin, argv, {
    encoding: "utf8",
    maxBuffer: MB,
    ...opts,
  });
...
function enginesFromArgs(args, hermesHost) {
  ...
  if (!hasAb)
    out.push({ id: "qjs", bin: binPath("qjs-bench"), kind: "qjs", extra: [] });
  ...
  for (const e of args.engines) {
    if (e === "hermes") {
      const h =
        ...
        hermesHost?.hermes ||
        "/Users/ammarahmed/Work/hermes/hermes/build_release/bin/hermes";
      if (h && existsSync(h))
        out.push({ id: "hermes", bin: h, kind: "hermes", extra: ["-O"] });
...
function pinnedFromBaseline(baseline, suite, row) {
  ...
  for (const r of baseline.rows) {
    if (r.name === row && (r.suite === suite || !r.suite) && r.pinned_iters)
      return r.pinned_iters;
  }
  return null;
}
...
function runWorkload(suite, row, engine, args, baselineIters) {
  ...
  const combined =
    harness.replace(
      "var __SINK__",
      `var __BENCH_PINNED_ITERS__ = ${pinned};\nvar __SINK__`,
    ) +
    src +
    tail;
  const tmp = join(
    tmpdir(),
    `rnqjs-bench-${process.pid}-${Math.random().toString(36).slice(2)}.js`,
  );
  ...
  const scan = (s) => {
    for (const line of (s || "").split("\n")) {
      if (line.startsWith("##BENCH## ")) {
        try {
          const b = JSON.parse(line.slice("##BENCH## ".length));
          if (b.name)
            results[b.name] = {
              ns: b.medNs,
              iters: b.iters,
        ...
        try {
          mem = JSON.parse(line.slice("##MEM## ".length));
        } catch {}
      }
    }
  };
...
function runOctane(suite, row, engine, args) {
  ...
  const argv = engine.extra.concat(["--stats"], wantMem ? ["--mem"] : [], [
    file,
  ]);
  const p = launch(engine, argv, { timeout: 300000 });
  if (p.status !== 0)
    return { error: "engine failed", stderr: (p.stderr || "").slice(0, 200) };
  let stats = null;
  let score = null;
  let mem = null;
  for (const line of [
  ...
  if (!score)
    return { error: "no Score: line", stderr: (p.stderr || "").slice(0, 200) };
...
function runKernels(suite, row, engine, args) {
  ...
  text = text.replace(
    /var CB_REPS = \d+;/,
    `var CB_REPS = ${Math.max(1, args.reps)};`,
  );
  const wantMem = args.mem || engine.kind === "qjs";
  const tmp = join(
    tmpdir(),
    `rnqjs-kernel-${process.pid}-${Math.random().toString(36).slice(2)}.js`,
  );
  ...
  if (p.status !== 0 && !(p.stdout || "").includes("##CB2##"))
    return {
      error:
        "engine failed: " +
        (p.stderr || "").split("\n").slice(0, 3).join(" | "),
  ...
  for (const line of [
    ...
    } else if (line.startsWith("##CBFAIL## "))
      return { error: line.slice("##CBFAIL## ".length) };
  }
...
function runCalls(suite, row, engine, args) {
  const bench = engine.device
    ? engine.device.calls
    : binPath("native-call-bench");
  const N1 = 100000,
    N2 = 1000000;
  function timeAt(n) {
    const p = engine.device
      ? device.deviceCallRun(bench, [row, String(n)])
      : spawnSync(bench, [row, String(n)], {
          encoding: "utf8",
...
function runStartup(suite, row, engine, args, baselineIters) {
  if (!args.n || !existsSync(args.n))
    return { error: "startup needs a bundle/file (--n <path>)" };
  let best = Infinity,
    stats = null;
...
async function main() {
  ...
  if (args.list) {
    ...
    for (const s of suites) {
      console.log(`\n[${s.name}]`);
      for (const r of filtered.filter((x) => x.suite === s.name))
        console.log("  " + r.id);
    }
  ...
  if (args.engines.includes("hermes") && !process.env.HERMES_BIN) {
    try {
      hermesHost = await hermes.ensureHostHermes();
    } catch (err) {
      console.error(`bench: hermes unavailable: ${err.message}`);
    }
  }
  ...
  if (args.device) {
    try {
      ...
      fail(`device: ${err.message}`);
    }
  }
  const baseline = loadBaseline(args.against || args.ab_baseline);
  const suites = selectedSuites(args);
  if (
    suites.some((s) => s.kind === "octane") &&
    !existsSync(join(OCTANE, "corpus", "MANIFEST.json"))
  ) {
    process.stderr.write("[bench] building octane corpus...\n");
    execFileSync(process.execPath, [join(OCTANE, "build-octane.mjs")], {
      stdio: "inherit",
    });
  }
  ...
  function pinnedMapForFile(baseline, suiteName) {
    ...
    let any = false;
    for (const r of baseline.rows) {
      if (r.suite === suiteName && r.pinned_iters) {
        m[r.name] = r.pinned_iters;
        any = true;
      }
    }
    return any ? m : null;
  ...
  let counter = 0,
    total = rows.length * engines.length;
  for (const s of suites) {
    const suite = s;
    ...
    if (suite.kind === "workload") {
      ...
      for (const e of engines) {
        ...
        process.stderr.write(
          `\r[${++counter}/${total}] ${s.name} on ${e.id}   `,
        );
        const r0 = run(suite, null, e, args, pinned);
        if (r0.error) {
          for (const r of suiteRows)
            perEngine[e.id][r.id] = { error: r0.error };
        } else {
          for (const r of suiteRows) {
            const res = r0.results && r0.results[r.name];
            perEngine[e.id][r.id] = res
              ? { ...res, mem: r0.mem }
              : { error: "row not produced" };
          }
        }
      }
    } else {
      for (const r of suiteRows) {
        for (const e of engines) {
          const pinned = baseline
            ? pinnedFromBaseline(baseline, r.suite, r.name)
            : null;
          process.stderr.write(
            `\r[${++counter}/${total}] ${r.id} on ${e.id}   `,
          );
  ...
  const memOn = args.no_mem
    ? false
    : args.mem || engines.some((e) => e.kind === "qjs");
  const rowMetric = (r, v) =>
    v.score ? "score" : r.suite === "startup" ? "ms" : "ns";
  ...
  for (const e of engines) {
    ...
    for (const r of rows) {
      ...
      if (v.error)
        console.log(`  ${r.id.padEnd(idW)}  ${"(error: " + v.error + ")"}`);
      else if (m === "score")
        console.log(`  ${r.id.padEnd(idW)}  ${String(v.score).padStart(numW)}`);
      else if (m === "ms")
        console.log(
          `  ${r.id.padEnd(idW)}  ${v.ms != null ? v.ms.toFixed(2) + " ms" : "-"}`,
        );
      else
        console.log(
          `  ${r.id.padEnd(idW)}  ${v.ns != null ? v.ns.toFixed(1) : "-"}  ${v.spread != null ? "spread=" + v.spread.toFixed(1) + "%" : ""}  iters=${v.iters || "-"}`,
        );
    }
    const scores = rows
      .map((r) => perEngine[e.id][r.id])
      .filter((v) => v && v.score > 0)
      .map((v) => v.score);
    if (scores.length > 1)
      console.log(
        `  ${"geomean".padEnd(idW)}  ${geomean(scores).toFixed(1).padStart(numW)}`,
      );
  }
  ...
  if (engines.length > 1) {
    ...
    let ratios = [];
    for (const r of rows) {
      const a = perEngine[ref][r.id],
        b = perEngine[engines[1].id][r.id];
    ...
    if (ratios.length)
      console.log(
        `  ${"geomean".padEnd(40)}  ${geomean(ratios.map((r) => r.ratio)).toFixed(3)}x`,
      );
  }
  ...
  if (memOn) {
    ...
    for (const e of engines) {
      ...
      const m = first.mem;
      console.log(
        `  ${e.id.padEnd(10)}  malloc=${(m.mallocSize / 1024).toFixed(0)}KB  obj=${m.objCount}  prop=${m.propCount}  shape=${m.shapeCount}  str=${m.strCount}  atom=${m.atomCount}`,
      );
    }
  }
  ...
  if (args.save) {
    const out = {
      rows: [],
      engines: engines.map((e) => ({ id: e.id, bin: e.bin })),
    };
    for (const r of rows) {
      const o = {
        id: r.id,
    ...
    const p =
      typeof args.save === "string" ? args.save : join(HERE, "baseline.json");
  ...
  if (args.against) {
        ...
        const bMetric =
          m === "score"
            ...
            : m === "ms"
              ...
              : (b[e.id] || {}).ns;
        const curMetricVal =
          m === "score" ? cur.score : m === "ms" ? cur.ms : cur.ns;
        if (bMetric > 0 && curMetricVal > 0) {
          const r0 = curMetricVal / bMetric;
          if (m === "score" ? r0 < 0.97 : r0 > 1.05) {
            console.error(
              `REGRESS ${r.id} on ${e.id}: ${bMetric.toFixed(2)} -> ${curMetricVal.toFixed(2)} (${r0.toFixed(3)}x)`,
            );
            regressions++;
          }
        }
    ...
    if (regressions > 0) {
      console.error(`${regressions} regression(s)`);
      process.exit(1);
    }
