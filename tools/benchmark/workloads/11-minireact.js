/*
 * MiniReact — Meta's own React-shaped benchmark, ported to this harness.
 *
 * Source: hermes/benchmarks/MiniReact/no-objects/out/simple-lowered.js
 * (Copyright (c) Meta Platforms, Inc. and affiliates, MIT licensed). It is a
 * self-contained, zero-dependency React implementation — elements, fibers,
 * useState, a keyed child reconciler, a microtask queue — plus a 43-element
 * app that exercises it, lowered by Meta's own toolchain to plain JS.
 *
 * The reconciler and app code below are kept VERBATIM. Its value is precisely
 * that it is Meta's artifact and not ours: it is the workload Hermes's own
 * team used to reason about React performance, so it cannot have been shaped
 * to make any engine here look good. Only the driver at the bottom is ours —
 * the original ran 100 warmup + 10,000 timed iterations and printed its own
 * elapsed time, which the harness does better (calibration, repeats, min
 * rather than mean, and a correctness gate).
 *
 * One iteration = createRoot + render + 2 event dispatches + microtask drain +
 * re-render, over 43 JSX elements.
 *
 * Published numbers for the original 10,000-iteration loop:
 *
 *   Hermes       317 ms   (~31.7 µs/iteration)
 *   V8            81 ms   (~8.1 µs/iteration)
 *   V8 jitless   503 ms   (~50.3 µs/iteration)
 *
 * The V8-jitless row is the one to read first. Interpreter-to-interpreter is
 * the comparison that means something for React Native, where JIT is not on
 * the table, and on that comparison Hermes was already ahead.
 *
 * Note this uses `let`/`const`, classes, template literals, object spread and
 * `??` — unlike the other workloads here it is not ES5. That is deliberate:
 * it is what Meta shipped, and both engines must handle it.
 */
...
(function () {
  ...
  /* ---------------------------------------------------------------- */
  /* Harness driver (ours — replaces the original's timing loop)       */
  /* ---------------------------------------------------------------- */

  /*
   * One iteration, byte-for-byte the body of the original's timed loop
   * (app/simple/index.js). The original's separate 100-iteration warmup is
   * dropped because the harness calibrates and warms up on its own.
   *
   * Note the second `update` with the same element is intentionally a no-op in
   * MiniReact (it compares element identity); the actual re-render happens
   * inside drainMicrotaskQueue, driven by the setState the two dispatched
   * events queued. That is the original's shape and it is left alone.
   */
  ...
  bench({
    name: 'minireact/render',
    unit: 'iteration',
    minMs: 200,
    run: oneIteration,
    /*
     * The gate renders the resulting fiber tree to the string form MiniReact
     * itself prints, and checks it is the exact tree the original produces
     * after both events land: the modal opened by the click, and the typed
     * text propagated into both the input and the textarea. It runs once,
     * outside the timed region, so the (expensive) toString costs nothing.
     */
    expect: function (root) {
      const out = root.toString();
      return out.length === 1053 && out.indexOf('!!!!! some text !!!!!') !== -1;
    }
  });
})();
