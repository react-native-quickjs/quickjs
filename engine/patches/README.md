...
| `0011` | Experimental lazy JSON.parse markers with shared source backing and container bulk completion | Large payloads are often only partly inspected; eager parsing pays for values that are never read, while the earlier accessor prototype allocated one getter and setter per property | - | No |
